...
app.post("/signup", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ data: null, message: "User register successfully" });
  } catch (err) {
    console.log(err);
    res.status(500).json({ data: null, message: err.message });
  }
...
app.post(
  ...
  async (req, res) => {
    try {
      const { hotelemail, hotellocation, hotelname, hotelphone, hotelrent } =
        req.body;
      const obj = {
        hotelname: hotelname,
      ...
      res
        .status(200)
        .json({ data: null, message: "Form submitted successfully" });
    } catch (err) {
      console.log(err);
      res.status(500).json({ data: null, message: err.message });
    }
...
app.put(
  ...
  async (req, res) => {
    try {
      const id = req.params.id;
      console.log(req.body);
      let checkImage = req.files["hotelimage"]
        ? req.files["hotelimage"][0].filename
        : false;
